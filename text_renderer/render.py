...
class Render:
    ...
    def gen_single_corpus(self) -> Tuple[PILImage, str, PILImage, PILImage]:
        ...
        bg_st = ImageStat.Stat(bg)

        if self.cfg.text_color_cfg is not None:
            text_color = self.cfg.text_color_cfg.get_color(bg)

            text_mean = sum(text_color) / 3
            bg_mean = sum(bg_st.mean) / 3

            # if abs(text_mean - bg_mean) < 15:
            #     print("value of mean pixel is too similar. Ignore this image")

            raise "resized_img_st \n {}".format(text_mean)
            print("background_img_st \n {}".format(bg_mean))
        
            raise PanicError("value of mean pixel is too similar. Ignore this image")
        ...
        if self.corpus.cfg.text_color_cfg is not None:
            ...
            raise PanicError("Ignore this image")
